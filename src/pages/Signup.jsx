      ...
      <div className='w-auto sm:w-[30rem] mx-auto bg-signin flex flex-col justify-center items-center py-[1.5rem] px-[2.5rem] mb-[2rem] mt-[7rem] shadow-lg rounded-2xl border-border border-[1px]'>
        ...
        <div className='flex w-full flex-col items-center'>
          ...
          <div className='flex w-full gap-[0.5rem]'>
            <button
              className={`flex w-full justify-center items-center mt-[1.5rem] font-Regular text-txt-gray-black border-border border-[1px] py-[0.5rem] px-[0.9rem] rounded-xl cursor-pointer hover:border-txt-black focus:bg-btn-blue focus:text-nav-white focus:border-btn-blue ${role === 'founder' ? 'bg-btn-blue border-btn-blue text-white' : ''} ${role === 'founder' ? 'text-nav-white' : ''}`}
              onClick={() => setRole('Sign Up as a Founder')}
            >Join as a Founder</button>
            <button
              className={`flex w-full justify-center items-center mt-[1.5rem] font-Regular text-txt-gray-black border-border border-[1px] py-[0.5rem] px-[0.9rem] rounded-xl cursor-pointer hover:border-txt-black focus:bg-btn-blue focus:text-nav-white focus:border-btn-blue ${role === 'investor' ? 'bg-btn-blue border-btn-blue text-white' : ''} ${role === 'investor' ? 'text-nav-white' : ''}`}
              onClick={() => setRole('Sign Up as an Investor')}
            >Join as an Investor</button>
          </div>
          <button
            className={`flex w-full justify-center items-center mt-[1.5rem] font-Regular text-txt-gray-black border-border border-[1px] py-[0.5rem] px-[0.4rem] rounded-xl ${role === "Sign Up as a Founder" || role === "Sign Up as an Investor" ? "cursor-pointer hover:border-txt-black" : "cursor-not-allowed opacity-50 hover:border-border"}`}
          ...
          <form onSubmit={handleFormSubmit} className='w-full flex flex-col justify-center items-center'>
            ...
            <button type='submit' className={`font-Regular mt-[1.5rem] bg-border text-nav-white w-full py-[0.5rem] text-center rounded-lg cursor-not-allowed transition ${signupEnabled ? "cursor-pointer bg-btn-blue" : "cursor-not-allowed"}`}>
              {role}
