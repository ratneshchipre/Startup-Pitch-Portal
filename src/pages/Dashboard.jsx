    ...
    return (
        <div className='flex flex-col mini-desktop:flex-row w-full items-center'>
            <div className='fixed flex gap-[1.2rem] items-center w-full top-0 py-[1rem] px-[1rem] mini-desktop:px-[2rem] z-100 bg-nav-white border-b-dash-border border-b-[2px]'>
                ...
                <h1 className='font-Medium text-[1.3rem] text-txt-black select-none'>Welcome, {userName}!</h1>
            </div>
                ...
                <div className='flex flex-col items-center px-[1rem] py-[1.3rem]'>
                    <div className='w-full'>
                        <ul className='font-Medium flex flex-col text-features gap-[0.4rem]'>
                            <li>
                                <Link to={`${role === 'founder' ? '/account/founder/profile' : '/account/investor/profile'}`} onClick={() => setIsOpen(false)}>
                            ...
                            <li>
                                <Link to={`${role === 'founder' ? '/account/founder/create-pitch' : '/account/investor/find-pitches'}`} onClick={() => setIsOpen(false)}>
                                    <button className='flex items-center w-full gap-[1.2rem] px-[1.5rem] py-[0.7rem] rounded-lg hover:bg-blue-50 hover:text-btn-blue cursor-pointer transition-all focus:bg-blue-50 focus:text-btn-blue'>
                                        {role === 'founder' ? <LuUpload /> : <FaMagnifyingGlass />}
                                        <span className='text-[1.1rem]'>{role === 'founder' ? 'Upload Pitch' : 'Find Pitches'}</span>
                                    </button>
                                </Link>
                            </li>
                            <li>
                                <Link to={`${role === 'founder' ? '/account/founder/my-pitches' : '/account/investor/saved-pitches'}`} onClick={() => setIsOpen(false)}>
                                    <button className='flex items-center w-full gap-[1.2rem] px-[1.5rem] py-[0.7rem] rounded-lg hover:bg-blue-50 hover:text-btn-blue cursor-pointer transition-all focus:bg-blue-50 focus:text-btn-blue'>
                                        <FontAwesomeIcon icon={role === 'founder' ? faList : faHeart} className='' />
                                        <span className='text-[1.1rem]'>{role === 'founder' ? 'My Pitches' : 'Saved Pitches'}</span>
                                    </button>
                                </Link>
                            </li>
                            <li>
                                <Link to={`${role === 'founder' ? '/account/founder/analytics' : '/account/investor/my-investments'}`} onClick={() => setIsOpen(false)}>
                                    <button className='flex items-center w-full gap-[1.2rem] px-[1.5rem] py-[0.7rem] rounded-lg hover:bg-blue-50 hover:text-btn-blue cursor-pointer transition-all focus:bg-blue-50 focus:text-btn-blue'>
                                        <FontAwesomeIcon icon={faChartLine} className='' />
                                        <span className='text-[1.1rem]'>{role === 'founder' ? 'Analytics' : 'My Investments'}</span>
                                    </button>
                                </Link>
                            </li>
                            {role === 'founder' &&
